...
class DCGANTrainer:
    ...
    class Discriminator(nn.Module):
        def __init__(self, outer_instance):
            ...
            self.feature_extractor = resnet18(weights=None)  # No pretrained weights
            self.feature_extractor.fc = nn.Identity()  # Remove final classification layer

            # Define the adversarial output (real vs. fake)
            self.adv_layer = nn.Sequential(
                nn.Linear(512, 1),
                nn.Sigmoid()
            )

            # Define the defectiveness output (defect vs. non-defect)
        ...
        def forward(self, img):
            ...
            features = self.feature_extractor(img)  # Shape: [batch_size, 512]

            # Flatten the feature map
            features = features.view(features.size(0), -1)  # Flatten (batch_size, 512 * 8 * 8)


            # Get both outputs
            validity = self.adv_layer(features)     # Real/Fake
            defectiveness = self.class_layer(features)  # Defect/Non-defect

            return validity, defectiveness

    def train(self, val_data_path, val_ann_file_path):
        best_val_metric = -np.inf  # Best validation metric
    ...
    def save_model(self, discriminator, epoch, best=False, model_path="dcgan_resnet_models"):
        """
